...
function ProposalCard({ lead, biz }: { lead: any; biz: any }) {
  ...
  return (
    <article className="panel flex flex-col p-5 transition-colors hover:border-border-strong">
      ...
      <p className="mt-4 flex items-center gap-1.5 text-[11px] text-muted-foreground">
        ...
        {(proposal?.updated_at || lead.updated_at) ? new Date(proposal?.updated_at || lead.updated_at).toLocaleDateString() : "N/A"}
      </p>
                  ...
                  <div className="mt-4 space-y-4 text-sm">
                    {proposal.content.executive_summary && (
                      <div>
                        ...
                        <div className="whitespace-pre-wrap text-muted-foreground">{proposal.content.executive_summary}</div>
                      </div>
                    )}
                    
                    {proposal.content.sections && Array.isArray(proposal.content.sections) && proposal.content.sections.map((sec: any, idx: number) => (
                      <div key={idx}>
                        <div className="font-semibold text-base mb-1">{sec.heading || sec.title || `Section ${idx + 1}`}</div>
                        <div className="whitespace-pre-wrap text-muted-foreground">{sec.content || sec.body}</div>
                      </div>
                    ))}
                    ...
                    {proposal.content.pricing && (
                      <div>
                        ...
                        <div className="whitespace-pre-wrap text-muted-foreground">{typeof proposal.content.pricing === 'string' ? proposal.content.pricing : JSON.stringify(proposal.content.pricing, null, 2)}</div>
                      </div>
                    )}
                    ...
                    {proposal.content.next_steps && (
                      <div>
                        ...
                        <div className="whitespace-pre-wrap text-muted-foreground">{typeof proposal.content.next_steps === 'string' ? proposal.content.next_steps : JSON.stringify(proposal.content.next_steps, null, 2)}</div>
                      </div>
                    )}
