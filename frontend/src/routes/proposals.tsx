...
function Proposals() {
  ...
  const totalValue = activeProposals.reduce((sum, item) => sum + ((item.biz?.opportunity_score || 70) * 120), 0);
...
function ProposalCard({ lead, biz }: { lead: any; biz: any }) {
  ...
  const { data: proposal, isLoading: proposalLoading } = useProposal(biz?.slug || null, opp?.id || null);
  ...
  return (
    <article className="panel flex flex-col p-5 transition-colors hover:border-border-strong">
      ...
      <p className="mt-4 flex items-center gap-1.5 text-[11px] text-muted-foreground">
        <Sparkles className="size-3 text-primary" /> Generated by AI · updated {new Date(proposal?.updated_at || lead.updated_at).toLocaleDateString()}
      </p>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-border pt-4 mt-auto">
        ...
        <div className="flex gap-2">
          {proposal ? (
            <>
              <Button variant="ghost" size="sm" onClick={() => alert("Preview feature not fully implemented")}>
                Preview
              </Button>
            ...
            <Button size="sm" onClick={handleGenerate} disabled={isGenerating || !bi?.id}>
              {isGenerating ? <Loader2 className="size-3.5 mr-1.5 animate-spin" /> : <Sparkles className="size-3.5 mr-1.5" />}
